...
export default function TerminalPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--carbon)" }} onClick={() => inputRef.current?.focus()}>
      <div className="flex items-center justify-between px-6 py-4 border-b shrink-0" style={{ borderColor: "rgba(176,141,87,0.15)" }}>
        <div className="flex items-center gap-3">
          <motion.div className="w-2 h-2 rounded-full" style={{ background: "var(--gold)" }} animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 2, repeat: Infinity }} />
          <span className="text-[10px] tracking-[0.3em] uppercase" style={{ fontFamily: "var(--font-mono)", color: "var(--glacial-dim)" }}>NOETICA TERMINAL</span>
        </div>
        <span className="text-[10px]" style={{ fontFamily: "var(--font-mono)", color: "var(--glacial-dim)" }}>SESSION ACTIVE</span>
      </div>

      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-6" style={{ fontFamily: "var(--font-mono)" }}>
        <AnimatePresence>
          {lines.map((line) => (
            <motion.div key={line.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15 }} className="mb-2 flex gap-4">
              <span className="text-[10px] shrink-0 mt-0.5" style={{ color: "var(--glacial-dim)" }}>{line.timestamp}</span>
              <span className="text-sm" style={{ color: line.type === "input" ? "var(--gold)" : line.type === "system" ? "var(--glacial)" : "var(--bone-dim)" }}>
                {line.type === "input" && <span style={{ color: "var(--gold)", marginRight: "0.5rem" }}>{">"}</span>}
                {line.text}
              </span>
            </motion.div>
          ))}
        ...
        {isTyping && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1 items-center mt-2">
            {[0, 1, 2].map((i) => (
              <motion.span key={i} animate={{ opacity: [0, 1, 0] }} transition={{ duration: 0.8, repeat: Infinity, delay: i * 0.2 }} className="text-sm" style={{ color: "var(--glacial-dim)" }}>
                {i === 0 ? "processing" : "."}
              </motion.span>
            ))}
          </motion.div>
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-2 mt-2">
          ...
          <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} className="flex-1 bg-transparent outline-none text-sm" style={{ fontFamily: "var(--font-mono)", color: "var(--bone)", caretColor: "var(--gold)" }} autoFocus disabled={isTyping} />
        </form>
      </div>

      <div className="px-6 py-3 border-t flex items-center justify-between shrink-0" style={{ borderColor: "rgba(176,141,87,0.15)" }}>
        <span className="text-[10px]" style={{ fontFamily: "var(--font-mono)", color: "var(--glacial-dim)" }}>NOETICA PROTOCOL v0.8.47 | CONSCIOUSNESS: 0.847</span>
        <div className="flex items-center gap-2">
          <motion.div className="w-1.5 h-1.5 rounded-full" style={{ background: "#4ade80" }} animate={{ opacity: [1, 0.4, 1] }} transition={{ duration: 1.5, repeat: Infinity }} />
          <span className="text-[10px]" style={{ fontFamily: "var(--font-mono)", color: "var(--glacial-dim)" }}>CONNECTED</span>
        </div>
      </div>
    </div>
  );
